...
interface SimpleOptimizedPostListProps {
  viewMode: 'grid' | 'list'
  searchQuery?: string
...
export function SimpleOptimizedPostList({ 
  viewMode, 
  searchQuery = '', 
  selectedPostId, 
  onPostSelect, 
...
}: SimpleOptimizedPostListProps) {
  const { organization } = useAuth()
  const [posts, setPosts] = useState<Post[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)
  const [lastOrganizationId, setLastOrganizationId] = useState<string | null>(null)
  const [lastRefreshTrigger, setLastRefreshTrigger] = useState<number | undefined>(undefined)
  ...
  // Load posts when organization changes
  useEffect(() => {
  ...
  // Load posts when refresh trigger changes
  useEffect(() => {

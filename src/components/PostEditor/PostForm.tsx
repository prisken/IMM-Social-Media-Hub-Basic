...
import { Save, Eye, Calendar, Hash, Image, Video, Type, Send, AlertCircle, Plus } from 'lucide-react'
import { apiService } from '@/services/ApiService'
import { createMediaService } from '@/services/media/MediaService'
import { useAuth } from '@/components/Auth/AuthProvider'
import { PostFormData, Category, Topic } from '@/types'
...
export function PostForm({ selectedPostId, onPostSelect, onPostCreated }: PostFormProps) {
  ...
  const onSchedule = async (data: PostFormData) => {
    try {
      setIsLoading(true)
      setError(null)

      // Similar to onSubmit but with scheduling logic
      const hashtags = extractHashtags(data.content)
      setValue('hashtags', hashtags)

      console.log('Scheduling post:', { ...data, hashtags })
      ...
      <div className="p-4 border-b border-border">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Create and edit your content</p>
          </div>
